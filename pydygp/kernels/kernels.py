...
class Hyperparameter(namedtuple('Hyperparameter',
    ...
    specifcation of the parameter transform
    """
    __slots__ = ()
...
class KernelMixin:
    """Adds a tensor product operator to the base kernel in sklearn
    """
    def __matmul__(self, other):
        return DirectSum(self, other)

class OutputdimMixin:
    _n_outputs = 1

    def __add__(self, b):
        if hasattr(b, 'n_outputs'):
            b_n_outputs = b.n_outputs
            if b_n_outputs == self.n_outputs:
                return Sum(self, b)
            else:
                raise ValueError('Cannot add two kernels with different output dims')
        else:
            raise ValueError('b does not have an output dimension')
...
class WhiteKernel(OutputdimMixin, KernelMixin, sklearn_kernels.WhiteKernel):
    pass

class Sum(OutputdimMixin, sklearn_kernels.Sum):

    @property
    def n_outputs(self):
        return self.k1.n_outputs


class DirectSum(OutputdimMixin, KernelMixin, sklearn_kernels.KernelOperator):
    """
    Takes two kernels and returns the concatenated
    kernel over the disjoint sum of the spaces.
    """
    @property
    def n_outputs(self):
        return self.k1.n_outputs + self.k2.n_outputs

    def diag(self, X):
        return np.concatenate((self.k1.diag(X), self.k2.diag(X)))
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        K2 = self.k2(X, Y=Y)
        if eval_gradient:
            K1, dK1 = self.k1(X, Y=Y, eval_gradient=True)
            K2, dK2 = self.k2(X, Y=Y, eval_gradient=True)
            # pad redundant dimensions of dK with zeros
            _dK1 = np.zeros((K1.shape[0]+K2.shape[0],
                             K1.shape[1]+K2.shape[1],
                             dK1.shape[2]))
            _dK2 = np.zeros((K1.shape[0]+K2.shape[0],
                             K1.shape[1]+K2.shape[1],
                             dK2.shape[2]))
            _dK1[:K1.shape[0], :K1.shape[1], :] += dK1
            _dK2[K1.shape[0]:, K1.shape[1]:, :] += dK2
            K = block_diag(K1, K2)
            return K, np.dstack((_dK1, _dK2))
        else:
            return block_diag(K1, K2)

    def __repr__(self):
        return "{} {} {}".format(self.k1.__repr__(),
                                 u"\u2295",
                                 self.k2.__repr__())


# Utility functions for the LFM kernels
def wofz_grad(z):
...
class LFMOrder2Kernel(HyperparTransformMixin,
                      OutputdimMixin, sklearn_kernels.Kernel):
    ...
    def lf_crosscov(self, X, Y=None):
        ...
        U1 = Upsilon(t1, t2, gamc, length_scale)
        U2 = Upsilon(t1, t2, gam, length_scale)

        C = S * length_scale[None, :] * np.sqrt(np.pi) / (4 * 1j * w[:, None])

        return np.real((U1 - U2) * C[None, None, ...])
